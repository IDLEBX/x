...
from admin_panel import (
    ADMIN_IDS, admin_panel, admin_users_menu, admin_list_users, admin_users_next, admin_users_prev,
    admin_balance_menu, admin_add_balance_start, admin_deduct_balance_start, admin_handle_balance_input,
    admin_subscriptions_menu, admin_activate_sub_start, admin_deactivate_sub_start, admin_list_subs,
    admin_handle_subscription_input, admin_payment_methods_menu, admin_toggle_payment_method,
    admin_stats_menu, admin_find_user_start, admin_handle_find_user, admin_balance_user, admin_sub_user,
    admin_payment_requests, payment_next, payment_prev, confirm_payment, reject_payment
)
from subscription import (
    force_subscription_middleware, check_subscription_callback, subscription_menu, my_subscription_info,
    buy_balance_menu, buy_subscription_menu, payment_methods_menu, select_payment_method,
    send_receipt_request, handle_receipt, process_purchase, cancel_subscription
)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if upload_result == "CHILD_DETECTED":
            await wait_msg.edit_text("❌ تم رفض الصورة")
            return
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "upload":
        ...
        user = db.get_user(user_id)
        remaining = user.get('images_limit', FREE_IMAGES_LIMIT) - user.get('images_used', 0)
        keyboard = [[InlineKeyboardButton("📷 رفع صورة", callback_data="upload")], [InlineKeyboardButton("💰 الرصيد", callback_data="subscription_menu")]]
        if user_id in ADMIN_IDS:
            keyboard.insert(1, [InlineKeyboardButton("🔐 لوحة التحكم", callback_data="admin_panel")])
        ...
        await admin_users_next(update, context)
    elif data == "admin_users_prev":
        await admin_users_prev(update, context)
        ...
        await admin_payment_requests(update, context)
    elif data == "payment_next":
        await payment_next(update, context)
    elif data == "payment_prev":
        await payment_prev(update, context)
    elif data.startswith("confirm_payment_"):
        await confirm_payment(update, context)
    elif data.startswith("reject_payment_"):
        await reject_payment(update, context)
    elif data.startswith("admin_balance_user_"):
        await admin_balance_user(update, context)
    elif data.startswith("admin_sub_user_"):
        await admin_sub_user(update, context)
    elif data == "subscription_menu":
        await subscription_menu(update, context)
    elif data == "my_subscription":
        await my_subscription_info(update, context)
    ...
    elif data.startswith("select_method_"):
        await select_payment_method(update, context)
    elif data == "send_receipt":
        await send_receipt_request(update, context)
